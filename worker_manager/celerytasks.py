...
def _send_to_osf(doc):
    _send_to_resource_sync(doc.get('location'), doc.get('iso_timestamp'), 'created')
...
def _send_to_resource_sync(loc, mod, change):
    headers = {'content-type': 'application/xml'}
    payload = '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"xmlns:rs="http://www.openarchives.org/rs/terms/"><url><loc>{loc}</loc><lastmod>{mod}</lastmod><rs:md change="{change}"/></url></urlset>'.format(loc=loc, mod=mod, change=change)
